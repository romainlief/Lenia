    ...
    * Independent bound for each dimension::
    ...
    def __init__(
        ...
        if shape is not None:
            shape = tuple(shape)
            assert (
                isinstance(low, numbers.Number) or (isinstance(low, torch.Tensor) and low.shape == shape)
            ), "low.shape doesn't match provided shape"
            assert (
                isinstance(high, numbers.Number) or (isinstance(high, torch.Tensor) and high.shape == shape)
            ...
            if not isinstance(high, torch.Tensor):
                raise ValueError("high must be a torch.Tensor if not a number")
            shape = high.shape
            assert (
                isinstance(low, numbers.Number) or (isinstance(low, torch.Tensor) and low.shape == shape)
        ...
        if isinstance(low, numbers.Number):
            low = torch.full(shape, float(low), dtype=dtype)

        if isinstance(high, numbers.Number):
            high = torch.full(shape, float(high), dtype=dtype)
